...
vector<string> g;
int n,m;
bool vis[200][200][4][10];
int cc[200][200][4][10];
int DIR[4][2] = {{0,1},{1,0},{0,-1},{-1,0}};

void bfs(int i, int j, int k, int p) {
  FORZ(i,n) FORZ(j,m) FORZ(k,4) FORZ(p,10) vis[i][j][k][p] = false;
  queue<vector<int>> q;
  q.push({i,j,k,p});
  while (!q.empty()) {
...
void solve() {
  ...
  n = (int)g.size();
  m = (int)g[0].size();
  FORZ(i,n) FORZ(j,m) FORZ(k,4) FORZ(p,10) cc[i][j][k][p] = INT_MAX;
  cc[0][1][0][0] = g[0][1] - '0';
  cc[1][0][1][0] = g[1][0] - '0';
  int res = INT_MAX;
  while (true) {
    ...
    bfs(0,1,0,0);
    bfs(1,0,1,0);
    FORZ(k,2) FORZ(p,10) tmp = min(tmp, cc[n-1][m-1][k][p]);
    if (tmp == res) {
      break;
    }
    res = tmp;
